	...
	with tf.Session() as sess:
		...
		for i in range(20000):
			if(i % (train_num/batch_size) == 0):
				...
				print('cnt: ',i,sess.run(accuracy,feed_dict={X:test_data[:50],y_:test_label[:50],keep_prob:1.0}))
			start = int((i % (train_num/batch_size)) * batch_size)
			end = int((i % (train_num/batch_size) + 1) * batch_size)
			sess.run(train,feed_dict={X:train_data[start:end],y_:train_label[start:end],keep_prob:0.5})
			if(i % 200 == 199):
				print(sess.run(cross_entropy,feed_dict={X:train_data[start:end],y_:train_label[start:end],keep_prob:0.5}))
		print(sess.run(accuracy,feed_dict={X:test_data[start:end],y_:test_label[start:end],keep_prob:1.0}))
		'''